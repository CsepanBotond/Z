#összhangzattan #alteráltakkord 
Akkordcsoport; a basszus és valamely felső szólam hangjai között *b6* távolság van.

3 akkord tartozik a csoportba:
- $IV^6$
- $IV^6_5$
- $II^4_3$

Mivel a *b6*-es akkordok két irányból közelítik meg a D-t, ezért sokkal szorosabb kapcsolat jön létre a S és D funkciók között

Azonos nevű dúrban és mollban a *b6*-es akkordok megegyeznek, csak az alterációk számában van eltérés.

**A bővített szextes akkordok *állatorvosi lova***:
```music-abc
X:1 
L:1/4
M:2/4
%%score {(SAT) B}
K:C 
V:SAT
[EGc-] [CFc-] | [C^Fc-]/2 [D^Fc-]/2 [_E^Fc] | [D-GB-] [DFB] | [CEc]2
V:B clef=bass
"_C: I" C, "_IV6\nKK" A,, | "_IV6#\nb\nB" _A,,/2- "_II6#/4/3\nb\nB" _A,,/2- "_IV6#/5b\nb\nB" _A,, | "_V87" G,,- G,, | "_I" C,2 
```
# Feladat
- díszítés
- moduláció
# Funkció
S, oldása D

# Jelölése
B

$\underset{B}{\underset{\flat}{IV_{5\flat}^{6\sharp}}}=$ “bővített négy kvintszext”

# IV6
A $\underset{B}{IV^6}$ kvintkettőzött.
Ha $\underset{B}{IV^6}$ követi a diatonikus $IV^6$-et, akkor utóbbi – noha [[főhármasok szextakkordjai|főhármas]] – **mindig kvintkettőzött**. Alapkettőzés esetén *t8* párhuzamot kapnánk.

Dúrban a $\underset{B}{IV^6}$ basszushangja a leszállított 6. lépcső, mollban a diatonikus 6. lépcső. Utóbbi esetben ezért nem kell alterálni.

# IV6/5
Ha a $\underset{B}{IV^6_5}$-et V-re oldjuk, akkor *t5*-párhuzam jön létre a basszus és valamely felső szólam között, ami **megengedett** és *Mozart-kvint*nek hívjuk.

A $\underset{B}{IV^6_5}$ enharmonikus az 5 kvinttel lejjebb lévő hangnem $D^7$-ével, ezért kiváló eszköze a modulációnak.

# II4/3
Felépítése: *n3-n2-n3*.